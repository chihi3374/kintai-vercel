...
export async function POST(req: Request) {
  try {
    ...
    // 1. データ取得
    // ===========================
    const { storeToken, employeeId, employeeName, type } = await req.json();
    ...
    if (type === "clock_in") {
      ...
      // 今日の未退勤データ（E列の退勤時刻が空）が存在するかチェック
      const isAlreadyWorking = rows.some(
        (row) =>
          row[0] === date &&
      ...
      if (isAlreadyWorking) {
        return NextResponse.json(
          {
            success: false,
            error: "退勤していない勤務データがあるため、出勤できません",
          },
          { status: 400 }
        );
      }
      ...
      await sheets.spreadsheets.values.append({
        ...
        requestBody: {
          values: [[date, employeeId, employeeName, time, "", ""]],
        },
      });
    }
